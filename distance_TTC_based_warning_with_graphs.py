...
def plot_graphs(save_dir, frame_nums, pre_process_times, inference_times, post_process_times, deepsort_times, fps_values, avg_distance_values, avg_ttc_values):
    ...
    data = {
        ...
        'fps': fps_values,
        'avg_distance_m': avg_distance_values,
    ...
    # Plot 3: Average Distance and TTC Over Time
    axes[2].plot(df['frame'], df['avg_distance_m'], label='Average Distance (m)', color='blue')
    axes[2].plot(df['frame'], df['avg_ttc_s'], label='Average TTC (s)', color='orange')
    axes[2].set_xlabel('Frame Number')
    axes[2].set_ylabel('Value')
    axes[2].set_title('Average Distance and TTC Over Time')
    axes[2].legend()
    axes[2].grid(True)
...
# Color Legend Drawer
# -------------------------
def draw_color_legend(im0, show_distance=False, show_ttc=False):
    x, y = 10, 50
    h_step = 25
    ...
    if show_ttc:
        cv2.putText(im0, "Warning:", (x, y), font, scale, (255, 255, 255), thickness)
        cv2.rectangle(im0, (x + 100, y - 10), (x + 120, y + 10), (0, 0, 255), -1)
        cv2.putText(im0, "Imminent Collision", (x + 125, y), font, scale, (0, 0, 255), thickness)
        cv2.rectangle(im0, (x + 230, y - 10), (x + 250, y + 10), (0, 255, 255), -1)
        cv2.putText(im0, "Attention Required", (x + 255, y), font, scale, (0, 255, 255), thickness)
        cv2.rectangle(im0, (x + 360, y - 10), (x + 380, y + 10), (255, 255, 255), -1)
        cv2.putText(im0, "Safe", (x + 385, y), font, scale, (255, 255, 255), thickness)
        y += h_step

    if show_distance:
        cv2.putText(im0, "Distance (m) / TTC (s):", (x, y), font, scale, (255, 255, 255), thickness)
        cv2.rectangle(im0, (x + 170, y - 10), (x + 190, y + 10), (0, 0, 255), -1)
        cv2.putText(im0, "<=10m / <=2s", (x + 195, y), font, scale, (0, 0, 255), thickness)
        cv2.rectangle(im0, (x + 300, y - 10), (x + 320, y + 10), (0, 255, 255), -1)
        cv2.putText(im0, "11-50m / 3-10s", (x + 325, y), font, scale, (0, 255, 255), thickness)
        cv2.rectangle(im0, (x + 440, y - 10), (x + 460, y + 10), (255, 255, 255), -1)
        cv2.putText(im0, ">50m / >10s", (x + 465, y), font, scale, (255, 255, 255), thickness)
        y += h_step
...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
            ...
            if len(det):
                ...
                if len(outputs) > 0:
                    for j, (out, conf) in enumerate(zip(outputs, confs)):
                        ...
                        bbox_height = y2 - y1
                        k = 5000
                        distance = round(k / max(bbox_height, 1))
                        fixed_speed_mps = 5.0
                        ttc = distance / fixed_speed_mps if fixed_speed_mps > 0 else float('inf')
                        ...
                        if show_distance:
                            combined_text = f"Distance = {distance}m, TTC = {ttc:.1f}s, {warning_text}"
                            cv2.putText(im0, combined_text, (x1, y_offset),
        ...
        # Calculate average distance and TTC for the frame
        ...
        avg_distance_values.append(avg_distance)
        avg_ttc_values.append(avg_ttc)
        
        im0 = annotator.result()
        draw_color_legend(im0, show_distance=show_distance, show_ttc=show_ttc)
        ...
        if save_img:
            if vid_writer is None:
                ...
                h, w = im0.shape[:2]
                vid_writer = cv2.VideoWriter(output_path, cv2.VideoWriter_fourcc(*'mp4v'), fps_cap, (w, h))
    ...
    plot_graphs(save_dir, frame_nums, pre_process_times, inference_times, post_process_times, deepsort_times, fps_values, avg_distance_values, avg_ttc_values)
